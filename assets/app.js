...
/* v4.3.2 full: fixes .startsWith crash, adds storage migration, lobby click delegation, and guards */
const I18N = {
  zh: {
...
function applyI18N(){
  ...
  if(zhBtn) zhBtn.classList.toggle("ghost", LANG!=="zh");
  if(enBtn) enBtn.classList.toggle("ghost", LANG!=="en");
}
document.getElementById("btn-lang-zh").addEventListener("click", ()=>{
  LANG="zh"; localStorage.setItem("lang", LANG); applyI18N(); renderLobby(); if(STATE.currentGame) openBetPage(STATE.currentGame.code);
});
document.getElementById("btn-lang-en").addEventListener("click", ()=>{
  LANG="en"; localStorage.setItem("lang", LANG); applyI18N(); renderLobby(); if(STATE.currentGame) openBetPage(STATE.currentGame.code);
});
...
// ---- LocalStorage migration to avoid undefined fields from older prototypes ----
function migrateStorage(){
  let slip = JSON.parse(localStorage.getItem("slip")||"[]");
  let hist = JSON.parse(localStorage.getItem("betsHistory")||"[]");
  // normalize slips
  slip = slip.map(it=>{
    const type = (typeof it.type==="string" && it.type) ? it.type :
                 (Array.isArray(it.numbers) ? "P"+(it.numbers.length||1) : "BONUS_NUM");
...
function renderLobby(){
  ...
  list.forEach(g=>{
    ...
    el.innerHTML = `
      ...
      <div class="meta">
        <span class="mini">${miniLabel(g.model, g.freq)}</span>
...
function openBetPage(code){
  ...
  if(btnQuick) btnQuick.onclick = ()=>{
    const N = MODELS[g.model].N; const need = reqCount(); const s = new Set();
    while(s.size<need){ s.add(1+Math.floor(Math.random()*N)); }
    STATE.selection = Array.from(s);
    markGridSelection(); updateNote();
  };
  ...
  document.querySelectorAll("#bonus-oe-wrap .seg-btn").forEach(btn=>{
    btn.onclick=()=>{ document.querySelectorAll("#bonus-oe-wrap .seg-btn").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); STATE.selectionBonus=btn.getAttribute("data-oe"); updateNote(); };
  });
  document.querySelectorAll("#bonus-hl-wrap .seg-btn").forEach(btn=>{
    btn.onclick=()=>{ document.querySelectorAll("#bonus-hl-wrap .seg-btn").forEach(b=>b.classList.remove("active")); btn.classList.add("active"); STATE.selectionBonus=btn.getAttribute("data-hl"); updateNote(); };
  });
  ...
  for(let i=1;i<=N;i++){
    ...
    el.addEventListener("click", ()=>{
      ...
      if(at>=0){ STATE.selection.splice(at,1); el.classList.remove("active"); }
        ...
        const need=reqCount(); if(STATE.selection.length>=need) return;
