...
/**
 * v4.2.1: re-introduce mini rules strip on lobby cards:
 * e.g., "1-49, draw 6+bonus, Every 10m"
 * (label only; underlying timers unchanged)
 */
const I18N = {
  zh: {
...
const MODELS = {
  "49-6B": { N:49, K:7, label:"1-49, draw 6+bonus", quickPick:6 },
  "49-7":  { N:49, K:7, label:"1-49, draw 7",       quickPick:7 },
  "36-5":  { N:36, K:5, label:"1-36, draw 5",        quickPick:5 },
};
// Label for schedule (display only). By request: High shows "Every 10m".
...
// --- schedule & timers (unchanged from v4.2) ---
const NOW = ()=>Date.now();
function nextDrawOffset(freq){
  if(freq==="high") return 2*60*1000;    // prototype engine keeps 2m tick
  if(freq==="mid")  return 60*60*1000;   // 1h
  return 6*60*60*1000;                   // 6h (placeholder for "daily")
}
...
function renderLobby(){
  ...
  list.forEach(g=>{
    ...
    el.innerHTML = `
      ...
      <div class="meta">
        <span class="meta-mini"><span class="mini">${mini}</span></span>
...
function tick(){
  ...
  GAMES.forEach(g=>{
    ...
    if(st.phase==="open"){
      const sealMs = 60000;
      const remaining = st.nextDue - now;
      ...
      if(remaining<=0){
        ...
        st.reveal = [];
        reveal.innerHTML = "";
        badges.innerHTML = `<span class="badge orange">${t("drawing")}</span>`;
      }
      ...
      if(st.reveal.length < balls){
        if(!st._lastReve || now - st._lastReve >= interval){
          ...
          b.className = "reveal-ball";
          b.textContent = n;
          reveal.appendChild(b);
        }
      }else{
        st.phase="result";
        st.resultHoldUntil = now + 30000;
        badges.innerHTML = `<span class="badge">${t("result")}</span>`;
      }
    }else if(st.phase==="result"){
      if(now >= st.resultHoldUntil){
        st.phase="open";
        st.nextDue = now + nextDrawOffset(g.freq);
        reveal.innerHTML = "";
      }
    }
  });
...
function openBetPage(code){
  ...
  if(!g) return;
  STATE.currentGame = g;
  const title = document.getElementById("bet-title");
  const name = (LANG==="zh"?g.name_zh:g.name_en);
  title.textContent = `${name}`;
  // build number grid
  const grid = document.getElementById("number-grid");
  grid.innerHTML = "";
  const model = MODELS[g.model];
  const N = model.N;
  const want = model.quickPick;
  STATE.selection = [];
  for(let i=1;i<=N;i++){
    const el = document.createElement("div");
    el.className = "ball";
    ...
    el.addEventListener("click", ()=>{
      const v = i;
      const at = STATE.selection.indexOf(v);
      if(at>=0){ STATE.selection.splice(at,1); el.classList.remove("active"); }
      else {
        if(STATE.selection.length>=want) return;
  ...
  document.getElementById("btn-quick").onclick = ()=>{
    const nums = new Set();
    while(nums.size < want){
      nums.add(1 + Math.floor(Math.random()*N));
    }
    STATE.selection = Array.from(nums);
    document.querySelectorAll("#number-grid .ball").forEach(el=>{
      const v = parseInt(el.textContent,10);
      el.classList.toggle("active", STATE.selection.includes(v));
    });
    updateNote();
  };
  document.getElementById("btn-clear").onclick = ()=>{
    STATE.selection = [];
    document.querySelectorAll("#number-grid .ball").forEach(el=>el.classList.remove("active"));
    updateNote();
  };
...
function updateNote(){
  const want = MODELS[STATE.currentGame.model].quickPick;
  const note = document.getElementById("bet-note");
  note.textContent = `${t("selected")}: ${STATE.selection.length}/${want}`;
}

function renderSlip(){
  const ul = document.getElementById("slip-list");
  ul.innerHTML = "";
  if(!STATE.slip.length){
    const li = document.createElement("li");
    li.textContent = t("slip_empty");
    ul.appendChild(li);
    return;
  }
  STATE.slip.forEach((s, idx)=>{
    ...
    li.innerHTML = `<span>[${s.game.code}] ${name} — ${s.numbers.join(", ")} — $${s.stake}</span>
...
function addToSlip(){
  ...
  const want = MODELS[g.model].quickPick;
  if(STATE.selection.length !== want){ alert(`需要選滿 ${want} 個號碼`); return; }
  const stake = parseInt(document.getElementById("stake").value, 10);
  if(!(stake>0)){ alert("請輸入正確投注金額"); return; }
  STATE.slip.push({ game:g, numbers:[...STATE.selection].sort((a,b)=>a-b), stake });
  localStorage.setItem("slip", JSON.stringify(STATE.slip));
  renderSlip();
  STATE.selection = [];
  document.querySelectorAll("#number-grid .ball").forEach(el=>el.classList.remove("active"));
...
function submitBets(){
  if(!STATE.slip.length){ alert("沒有注單"); return; }
  alert("已送出 "+STATE.slip.length+" 筆注單（原型示意）");
  STATE.slip = [];
  localStorage.setItem("slip", JSON.stringify(STATE.slip));
  renderSlip();
}
